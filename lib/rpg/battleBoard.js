...
function renderBattle(
  ...
  return (
    `✦ *NEXA • BATTLE*\n` +
    `━━━━━━━━━━━━━━━━━━\n` +
    `👤 *${name}* • ${classInfo?.icon || '⚔️'} *${classInfo?.name || p.class || 'No Class'}* • Lv.${p.level}\n` +
    `❤️ ${hpState(p.hp, p.max_hp)} ${bar(p.hp, p.max_hp)}  ${p.hp}/${p.max_hp}\n` +
    `🔷 ${bar(p.mana, p.max_mana)}  ${p.mana}/${p.max_mana}\n\n` +
    `⚔️ *VS*\n` +
    `${b.monster_icon} *${String(b.monster_name).toUpperCase()}* • Lv.${b.monster_level}\n` +
    `❤️ ${hpState(b.monster_hp, b.monster_max_hp)} ${bar(b.monster_hp, b.monster_max_hp)}  ${b.monster_hp}/${b.monster_max_hp}\n\n` +
    `📜 *LAST TURN*\n` +
    `${last.replace(/^│ /gm, '')}\n\n` +
    `━━━━━━━━━━━━━━━━━━\n` +
    `🎮 *ACTION — reply board ini*\n` +
    `⚔️ attack  •  ✨ skill (${skillName(p.class)})\n` +
    `🧪 potion  •  🏃 run`
  )

// NEXA CHANNEL PROMO CONTEXT V2
// Resolve newsletter metadata ONLY after the WA socket is open.
...
export function installChannelPromo(
      ...
      (async () => {
        try {
          if (
            ...
            throw new Error(
              'newsletterMetadata() tidak tersedia di Baileys ini.'
            )
          }
          ...
          if (!found) {
            throw new Error(
              'Metadata saluran tidak berisi JID/nama yang valid.'
            )
          }
  ...
  function onConnectionUpdate(
    ...
    if (
      ...
      // Socket sudah authenticated/open.
      // Baru sekarang aman minta newsletter metadata.
      setTimeout(
        () => {
          void resolvePromo({
            force:
  ...
  sock.sendMessage =
    async function nexaSendMessage(
      ...
      // Jangan tahan respons bot hanya demi promo.
      // Kalau metadata belum siap, pesan tetap dikirim normal.
      if (!promo) {
        void resolvePromo()
